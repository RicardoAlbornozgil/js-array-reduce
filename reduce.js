...
function vowelCount(str) {
    const vowels = 'aeiou';
    return str.toLowerCase().split('').reduce(function (vowelObj, currentIndex) {
        if(vowels.includes(currentIndex)) {
            if (!vowelObj[currentIndex]) {
                vowelObj[currentIndex] = 1;
            } else {
                vowelObj[currentIndex] += 1;
            }
        }
        // console.log(JSON.stringify(vowelObj))
        return vowelObj
    }, {});
}
...
of the callback function at that specific value is false, the value should be placed in 
the second subarray. 
